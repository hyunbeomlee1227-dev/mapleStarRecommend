...
Blueprint는 `main` 브랜치의 GitHub Actions 검사가 통과한 커밋만 자동 배포합니다. Render의 관리형 프록시에서는 `TRUST_PROXY_MODE=render`를 사용해 애플리케이션에 가장 가까운 프록시 한 단계만 신뢰합니다. 다른 호스팅 환경에는 이 값을 설정하지 마세요.
