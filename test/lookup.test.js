...
test('Render proxy mode trusts only the final proxy hop', async () => {
  const { service } = setup();
  const app = createApp({ service, perMinute: 1, trustProxy: 1 });
  // Render appends the connecting client address as the entry nearest the app.
  const first = await request(app).get('/api/character?name=검증').set('X-Forwarded-For', 'spoofed-a, 10.0.0.1');
  const sameClientSpoofedAgain = await request(app).get('/api/character?name=검증').set('X-Forwarded-For', 'spoofed-b, 10.0.0.1');
  const otherClient = await request(app).get('/api/character?name=검증').set('X-Forwarded-For', 'spoofed-a, 10.0.0.2');
