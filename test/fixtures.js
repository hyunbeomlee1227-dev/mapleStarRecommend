export function responses(date = '2026-09-08') {
  return {
    ...
    equipment: { date, preset_no: 1, item_equipment: [{ item_name: '검증 장갑', item_equipment_slot: '장갑', starforce: '18', item_icon: 'https://open.api.nexon.com/static/maplestory/item/test.png', potential_option_grade: '레전드리', potential_option_1: '크리티컬 데미지 : +8%', item_total_option: { str: '150', base_equipment_level: 200 }, item_add_option: { str: '60', equipment_level_decrease: 0 } }] },
...
export function upstream(raw = responses(), override) {
  ...
  const fetchImpl = async (url, options) => {
    ...
    const key = { 'item-equipment': 'equipment', 'set-effect': 'set' }[path] || path;
