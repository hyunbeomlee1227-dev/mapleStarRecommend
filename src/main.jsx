...
function App() {
  ...
  useEffect(() => {
    ...
    const items = data.items.filter(supportsEnhancement).map(({ item_name, item_equipment_slot, item_equipment_part, item_total_option, item_base_option, starforce, special_ring_level, potential_option_grade, additional_potential_option_grade, potential_option_1, potential_option_2, potential_option_3 }) => ({
      item_name,
      item_equipment_slot,
      item_equipment_part,
  ...
  return <>
    ...
    <main className="workspace">
      ...
      <section className="equipment-recommendations" aria-label="보스 장비 강화 우선순위"><div className="equipment-recommendation-heading"><div><span>BOSS UPGRADE PRIORITY</span><h3>보스 장비 강화 우선순위</h3></div>{recommendationMode === 'budget' && <span className="budget-not-applied-token">예산 적용</span>}</div>
        ...
        <p className="recommendation-note">{recommendation.equipmentTargetTrace?.version || '목표 규칙 불러오는 중'} · 스타포스는 보유 스페어 기준 목표 별당 기대 메소 순이며 최종뎀 효율은 아직 적용되지 않습니다.</p>
      </section>
