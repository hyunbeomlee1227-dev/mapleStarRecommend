...
function PotentialRecommendationEvidence({ candidate }) {
  ...
  return <details className="potential-evidence"><summary>옵션별 판정{candidate.unconfirmedLines > 0 ? ` · 잠정 목표 · 미확인 ${candidate.unconfirmedLines}줄` : ''}</summary><ol>{candidate.lineAssessments.map((line) => <li key={line.line}><span>{line.line}줄 · {line.option || '옵션 미제공'}</span><strong>{labels[line.status] || '미판정'}</strong></li>)}</ol><p>유효 줄 수는 현재 규칙에 일치하는 후보 수이며 옵션별 성능이 같다는 뜻은 아닙니다.</p>{candidate.unconfirmedLines > 0 && <p>미판정·미제공 옵션은 성능이 없다는 뜻이 아닙니다. 이 추천은 확인이 필요한 잠정 목표입니다.</p>}</details>;
}
function StarforceEventStatus({ eventStatus }) {
  const candidate = eventStatus.candidates?.[0];
  if (eventStatus.status === 'verification-required' && candidate) {
    return <a className="event-status attention" href={candidate.sourceUrl} target="_blank" rel="noreferrer" aria-label="스타포스 기간 이벤트 확인">기간 이벤트 확인 필요<ArrowUpRight size={13} /></a>;
  }
  const label = eventStatus.status === 'loading' ? '기간 이벤트 확인 중' : eventStatus.status === 'none' ? '기간 이벤트 없음' : '기간 이벤트 확인 불가';
  return <span className={`event-status ${eventStatus.status}`}>{label}</span>;
}
...
function App() {
  ...
  const [mvpGrade, setMvpGrade] = useState('none');
  ...
  const [starforceEvents, setStarforceEvents] = useState({ status: 'loading', candidates: [] });
  ...
  useEffect(() => {
    ...
    fetch('/api/rules/starforce-events').then((response) => { if (!response.ok) throw new Error(); return response.json(); }).then(setStarforceEvents).catch(() => setStarforceEvents({ status: 'unavailable', candidates: [] }));
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [goalId, data, recommendationMode, budget, mvpGrade, pcRoom, safeguard]);
  ...
  const enhanceableItems = data.items.filter(supportsEnhancement);
  const items = data.items.filter((item) => `${item.item_name} ${item.item_equipment_slot}`.includes(filter)
    && (grade === 'all' || item.potential_option_grade === grade));
  ...
  const unsupportedStarforceItems = recommendation.supportedCalculations?.unsupportedStarforceItems ?? [];
  const equipmentRecommendations = recommendation.equipmentRecommendations ?? [];
  const budgetSummary = recommendation.equipmentTargetTrace?.budgetSummary;
  const budgetRemaining = recommendation.equipmentTargetTrace?.budgetRemaining;
  const jobEquipmentReference = recommendation.jobEquipmentReference;
  const effectiveStarforceEventStatus = recommendation.equipmentTargetTrace?.starforceEventStatus ?? starforceEvents.status;
  ...
  function displaySlot(item) {
    const matching = data.items.filter((candidate) => candidate.item_equipment_slot === item.item_equipment_slot && candidate.item_kind === item.item_kind);
    const slot = matching.length < 2 ? item.item_equipment_slot : `${item.item_equipment_slot} ${matching.indexOf(item) + 1}`;
    return isCashItem(item) ? `캐시 · ${slot}` : slot;
  }
