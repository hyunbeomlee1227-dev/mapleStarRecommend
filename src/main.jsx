...
function App() {
  ...
  useEffect(() => {
    ...
    fetch('/api/recommendations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ goalId, characterJob: data.character.job, mode: recommendationMode, budgetMesos, starforceConditions: { mvpGrade, pcRoom }, combat: data.combat, items }), signal: controller.signal })
      .then(async (response) => { const result = await response.json(); if (!response.ok) throw new Error(result.message); return result; })
      .then(setRecommendation)
      .catch((error) => { if (!controller.signal.aborted) setRecommendationStatus('unavailable', error.message || '추천 조건을 확인하지 못했습니다.'); });
    return () => controller.abort();
  }, [goalId, data, recommendationMode, budget, mvpGrade, pcRoom]);
  ...
  return <>
    ...
    <main className="workspace">
      ...
      <section className="equipment-recommendations" aria-label="보스 장비 강화 우선순위"><div className="equipment-recommendation-heading"><div><span>BOSS UPGRADE PRIORITY</span><h3>보스 장비 강화 우선순위</h3></div>{recommendationMode === 'budget' && <span className="budget-not-applied-token">예산 적용</span>}</div>
        {recommendation.status === 'loading' ? <p className="recommendation-empty">장비 목표를 확인하는 중입니다.</p> : equipmentRecommendations.length ? <div className="equipment-recommendation-list">{equipmentRecommendations.map((item) => <article key={`${item.ruleId}:${item.slot}:${item.itemName}`}><div className="equipment-recommendation-item"><div><small>{item.slot}</small><strong>{item.itemName}</strong></div><span>{item.actions.join(', ')}</span></div>{item.expectedMeso !== null && <div className="recommendation-cost"><strong>예상 {mesos.format(item.expectedMeso)} 메소</strong><span>별 1개당 {mesos.format(item.expectedMesoPerStar)} 메소</span>{item.expectedRecoveryCopies > 0 && <span>기대 소모 장비 {item.expectedRecoveryCopies.toFixed(2)}개</span>}</div>}<p>{item.reason}</p></article>)}</div> : <p className="recommendation-empty">현재 조건과 예산에서 추천할 강화가 없습니다.</p>}
        <p className="recommendation-note">{recommendation.equipmentTargetTrace?.version || '목표 규칙 불러오는 중'} · 스타포스는 보유 스페어 기준 목표 별당 기대 메소 순입니다. MVP·PC방 할인은 17성 도달까지 기본 강화 비용에만 적용되며 최종뎀 효율은 아직 적용되지 않습니다.</p>
      </section>
