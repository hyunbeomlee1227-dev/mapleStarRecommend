...
function App() {
  ...
  useEffect(() => {
    ...
    const items = data.items.filter((item) => item.item_kind !== 'cash').map(({ item_name, item_equipment_slot, item_equipment_part, item_total_option, item_base_option, starforce, special_ring_level, potential_option_grade, additional_potential_option_grade }) => ({
      item_name,
      item_equipment_slot,
      item_equipment_part,
  ...
  async function lookup(event, candidate = name) {
    ...
    try {
      ...
      setData(result); setSelected(result.items[0] || null); setFilter(''); setGrade('all'); setItemType('all'); setTab('equipment');
  ...
  function showDemo() {
    ...
    setRecommendation({ status: 'loading', message: '직업별 장비 표본을 확인하는 중입니다.' });
    setData(demo); setSelected(demo.items[0]); setFilter(''); setGrade('all'); setItemType('all'); setTab('equipment');
  }
  const enhanceableItems = data.items.filter((item) => item.item_kind !== 'cash');
  const items = data.items.filter((item) => `${item.item_name} ${item.item_equipment_slot}`.includes(filter)
    && (itemType === 'all' || (itemType === 'cash') === (item.item_kind === 'cash'))
  ...
  function displaySlot(item) {
    ...
    return item.item_kind === 'cash' ? `캐시 · ${slot}` : slot;
  }
