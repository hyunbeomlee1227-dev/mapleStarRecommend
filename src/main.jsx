...
function App() {
  ...
  const starTotal = data.items.reduce((sum, item) => sum + (Number(item.starforce) || 0), 0);
  ...
  return <>
    ...
    <main className="workspace">
      ...
      <section className="character-strip" aria-label="캐릭터 요약"><div className="character-identity"><div className="portrait">{data.character.image ? <img src={data.character.image} alt={isDemo ? '메이플스토리 안내 캐릭터' : `${data.character.name} 캐릭터`} onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : <Shield size={30} />}</div><div><span className="character-meta">{data.character.world} <span>·</span> Lv. {data.character.level}</span><h2>{data.character.name}</h2><span className="job"><Swords size={13} />{data.character.job}</span></div></div><div className="summary-stat"><span>착용 장비</span><strong>{data.items.length}<small>개</small></strong></div><div className="summary-stat"><span>스타포스 합계</span><strong><Star size={17} className="gold" />{starTotal}<small>성</small></strong></div><div className="summary-stat"><span>레전드리 잠재</span><strong>{data.items.filter((item) => item.potential_option_grade === '레전드리').length}<small>개</small></strong></div><div className="snapshot"><Clock3 size={14} /><span>{isDemo ? '예시 장비 프리셋' : `${data.date} 기준 (KST)`}<small>{data.preset ? `프리셋 ${data.preset}` : '프리셋 정보 없음'}{data.cached ? ' · 캐시된 조회' : ''}</small></span></div></section>
      <div className="tabs" role="tablist" aria-label="캐릭터 정보">{[['equipment', '장비 목록', Layers3], ['stats', '캐릭터 능력치', SlidersHorizontal], ['sets', '세트 효과', Shield]].map(([id, text, Icon]) => <button key={id} role="tab" aria-selected={tab === id} aria-controls={`panel-${id}`} id={`tab-${id}`} onClick={() => setTab(id)}><Icon size={16} />{text}{id === 'equipment' && <span className="count">{data.items.length}</span>}</button>)}</div>
