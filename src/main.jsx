...
function App() {
  ...
  return <>
    ...
    <main className="workspace">
      ...
      <section className="equipment-recommendations" aria-label="보스별 장비 목표"><div className="equipment-recommendation-heading"><div><span>BOSS EQUIPMENT TARGET</span><h3>보스별 장비 목표</h3></div>{recommendationMode === 'budget' && <span className="budget-not-applied-token">예산 필터 미적용</span>}</div>
        {recommendation.status === 'loading' ? <p className="recommendation-empty">장비 목표를 확인하는 중입니다.</p> : equipmentRecommendations.length ? <div className="equipment-recommendation-list">{equipmentRecommendations.map((item) => <article key={item.ruleId}><div className="equipment-recommendation-item"><div><small>{item.slot}</small><strong>{item.itemName}</strong></div><span>{item.actions.join(', ')}</span></div><p>{item.reason}</p></article>)}</div> : <p className="recommendation-empty">이 보스 구간에 등록된 장비 목표가 없습니다.</p>}
        <p className="recommendation-note">{recommendation.equipmentTargetTrace?.version || '목표 규칙 불러오는 중'} · 보스 장비 목표이며 최종뎀 및 메소당 효율 순위는 아직 적용되지 않습니다.</p>
      </section>
