...
function App() {
  ...
  useEffect(() => {
    ...
    const items = data.items.filter(supportsEnhancement).map(({ item_name, item_equipment_slot, item_equipment_part, item_total_option, item_base_option, starforce, special_ring_level, potential_option_grade, additional_potential_option_grade }) => ({
      item_name,
      item_equipment_slot,
      item_equipment_part,
  ...
  const items = data.items.filter((item) => `${item.item_name} ${item.item_equipment_slot}`.includes(filter)
    && (itemType === 'all' || (itemType === 'cash') === isCashItem(item))
