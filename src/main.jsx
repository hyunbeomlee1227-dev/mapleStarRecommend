...
function App() {
  ...
  const detail = selected ? <>
    ...
    <details className="breakdown"><summary>장비 강화 상세</summary>{[['기본 옵션', selected.item_base_option], ['추가옵션', selected.item_add_option], ['업그레이드 증가량', selected.item_etc_option], ...(supportsStarforce(selected) ? [['스타포스 증가량', selected.item_starforce_option]] : [])].map(([title, entries]) => <div key={title}><h4>{title}</h4><dl className="stat-list">{Object.entries(entries || {}).filter(([, value]) => value !== '0' && value != null).map(([key, value]) => <div key={key}><dt>{labels[key] || key}</dt><dd>{value}</dd></div>)}</dl>{!entries && <p className="muted">정보 없음</p>}</div>)}</details>
