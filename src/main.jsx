...
function OptionLines({ item, additional = false }) {
  return <ul className="option-lines">{[1, 2, 3].map((n) => <li key={n}>{item[`${additional ? 'additional_' : ''}potential_option_${n}`] || '정보 없음'}</li>)}</ul>;
}
function StatList({ entries }) {
  const values = Object.entries(entries || {}).filter(([, value]) => value !== '0' && value != null);
  if (!values.length) return <p className="muted option-empty">정보 없음</p>;
  return <dl className="stat-list">{values.map(([key, value]) => <div key={key}><dt>{labels[key] || key}</dt><dd>{value}</dd></div>)}</dl>;
}
function App() {
  ...
  const detail = selected ? <>
    ...
    <div className="selected-item"><EquipmentIcon item={selected} /><h3>{selected.item_name}</h3>{supportsStarforce(selected) && <span className="star-value"><Star size={14} fill="currentColor" /> {selected.starforce}성</span>}</div>
    <section className="detail-section item-options-overview"><div className="section-title"><h4>장비 옵션</h4><span className="option-scope">전체 보기</span></div>
      <div className="option-group potential-group"><div className="section-title"><h4>잠재능력</h4><Badge grade={selected.potential_option_grade} /></div><OptionLines item={selected} /><button className="official-options-button" disabled={!canLookupPotentialOptions(selected)} onClick={openPotentialOptions}>공식 잠재 옵션표 보기<ArrowUpRight size={13} /></button></div>
      <div className="option-group"><div className="section-title"><h4>에디셔널 잠재능력</h4><Badge grade={selected.additional_potential_option_grade} /></div><OptionLines item={selected} additional /></div>
      {[['총 옵션', selected.item_total_option], ['기본 옵션', selected.item_base_option], ['추가옵션', selected.item_add_option], ['업그레이드 증가량', selected.item_etc_option], ...(supportsStarforce(selected) ? [['스타포스 증가량', selected.item_starforce_option]] : [])].map(([title, entries]) => <div className="option-group" key={title}><h4>{title}</h4><StatList entries={entries} /></div>)}
    </section>
  </> : <p className="muted">조회된 장비가 없습니다.</p>;
  return <>
    ...
    <main className="workspace">
      ...
      <section className="equipment-recommendations" aria-label="보스별 장비 목표"><div className="equipment-recommendation-heading"><div><span>ADMIN EQUIPMENT TARGET</span><h3>보스별 장비 목표</h3></div>{recommendationMode === 'budget' && <span className="budget-not-applied-token">예산 필터 미적용</span>}<span className="admin-token">관리자 기준</span></div>
        {recommendation.status === 'loading' ? <p className="recommendation-empty">장비 목표를 확인하는 중입니다.</p> : equipmentRecommendations.length ? <div className="equipment-recommendation-list">{equipmentRecommendations.map((item) => <article key={item.ruleId}><div className="equipment-recommendation-item"><div><small>{item.slot}</small><strong>{item.itemName}</strong></div><span>{item.actions.join(', ')}</span></div><p>{item.reason}</p></article>)}</div> : <p className="recommendation-empty">이 보스 구간에 등록된 관리자 장비 목표가 없습니다.</p>}
        <p className="recommendation-note">{recommendation.equipmentTargetTrace?.version || '목표 규칙 불러오는 중'} · 관리자 장비 목표이며 최종뎀 및 메소당 효율 순위는 아직 적용되지 않습니다.</p>
      </section>
