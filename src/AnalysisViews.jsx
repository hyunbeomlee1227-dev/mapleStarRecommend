...
export function StarforceView({ risks }) {
  return <div className="analysis-view"><div className="view-heading"><div><span className="eyebrow">STAR FORCE</span><h3>스타포스 강화</h3></div><span>{risks.length}개 장비 계산 가능</span></div>{risks.length ? <div className="tier-table starforce-table"><div className="tier-head"><span>장비</span><span>현재</span><span>1회 비용</span><span>다음 성 기대 메소</span><span>성공</span><span>파괴</span><span>온전 복구</span></div>{risks.map((risk, index) => <div className="tier-row" key={`${risk.slot}-${risk.itemName}-${index}`}><strong>{risk.itemName}<small>{risk.traceRecoveryStar === null ? '파괴 없음' : `파괴 시 ${risk.traceRecoveryStar}성 흔적`}</small></strong><span className="star-value"><Star size={12} fill="currentColor" />{risk.currentStar}성</span><span>{mesos.format(risk.attemptCost)}</span><span>{mesos.format(risk.expectedMesoWithoutSpares)} 메소</span><span>{(risk.successProbability * 100).toFixed(3).replace(/0+$/, '').replace(/\.$/, '')}%</span><span>{(risk.destroyProbability * 100).toFixed(3).replace(/0+$/, '').replace(/\.$/, '')}%</span><span>{risk.intactRecoveryCopies === null ? '-' : `동일 장비 ${risk.intactRecoveryCopies}개`}</span></div>)}</div> : <p className="view-empty">스타포스 확률을 계산할 수 있는 장비가 없습니다.</p>}<p className="calculation-note">스타포스 적용이 불가능한 장비는 이 화면에서 제외합니다. 이벤트·할인·파괴방지와 스페어 가격은 포함하지 않습니다.</p></div>;
}
